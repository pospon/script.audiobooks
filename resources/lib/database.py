...
class AudioBooksDB():
    ...
    def _createDatabase(self):
        ...
        if not xbmcvfs.exists(self.databasefile):
            ...
            c.execute('''CREATE TABLE books (id integer primary key, fullpath text unique, title text, num_chapters integer, position text, complete integer)''')
    ...
    def addAudioBook(self, fullPath, title, numChapters=0):
        ...
        cmd = 'INSERT OR REPLACE INTO books (fullpath, title, num_chapters, position, complete) VALUES (?,?,?,"",0)'
